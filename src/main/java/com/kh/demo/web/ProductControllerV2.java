...
public class ProductControllerV2 {
  ...
  public String add(
    ...
    //1-2)수량
    if (!Pattern.matches(pattern, addForm.getQuantity())) {
      model.addAttribute("addForm", addForm);
      model.addAttribute("s_err_quantity","영문/숫자/한글/_-가능, 3~10자리");
      return "productv2/add";
    }
    //1-3)가격
    if (!Pattern.matches(pattern, addForm.getPrice())) {
      model.addAttribute("addForm", addForm);
      model.addAttribute("s_err_price","영문/숫자/한글/_-가능, 3~10자리");
      return "productv2/add";
    }
    //상품등록
