...
//  @ResponseBody
  @GetMapping("/{pid}")
  public ApiResponse<?> findByTextId(@PathVariable("pid") Long pid){
    log.info("pid={}", pid);
    Optional<Tester> optionalTester = productSVC.findByTextId(pid);

    ApiResponse<ResFindById> res = null;
    //상품을 찾은경우
    if(optionalTester.isPresent()){
      Tester findedTester = optionalTester.get();

      ResFindById resFindById = new ResFindById();
      BeanUtils.copyProperties(findedTester,resFindById);
...
//  }
  ...
  public ApiResponse<?> list(
    ...
    List<Tester> list = productSVC.findTextAll(reqPage,reqCnt);

    ApiResponse<List<Tester>> res = null;
